...
'''
LABEL = r'''from __future__ import annotations
import hashlib
...
'''
VAR={"baseline_adele_lite_eta003":.03,"baseline_adele_lite_eta005":.05,"baseline_adele_lite_eta008":.08,"baseline_adele_lite_eta010":.10,"baseline_adele_lite_eta015":.15}
def project():
    here=Path.cwd().resolve()
    for p in [here,here/"projects"/"predictive_eval_challenge",Path(__file__).resolve().parent]:
        if (p/"codabench_submissions").exists(): return p
    raise FileNotFoundError("Run from repo root or place in project folder")
def zipdir(src,out):
...
def main():
    proj=project(); sub=proj/"codabench_submissions"; art=sub/"adele_lite_modal"/"artifacts"
    if not (art/"smoothed_prior.json").exists() or not (art/"adele_lite_model.npz").exists(): raise FileNotFoundError(f"Missing artifacts in {art}")
    dist=proj/"dist"; dist.mkdir(parents=True,exist_ok=True)
    for name,eta in VAR.items():
        dst=sub/name
        if dst.exists(): shutil.rmtree(dst)
        (dst/"artifacts").mkdir(parents=True,exist_ok=True)
        (dst/"model.py").write_text(MODEL.replace("__ETA__",repr(float(eta))).lstrip(),encoding="utf-8")
        (dst/"labeling.py").write_text(LABEL.lstrip(),encoding="utf-8")
        (dst/"models.txt").write_text("",encoding="utf-8")
        shutil.copyfile(art/"smoothed_prior.json",dst/"artifacts"/"smoothed_prior.json")
        shutil.copyfile(art/"adele_lite_model.npz",dst/"artifacts"/"adele_lite_model.npz")
        out=dist/f"{name}_submission.zip"; zipdir(dst,out); print(f"READY: {out}")

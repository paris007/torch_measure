...
\text{global}.
\]
The first baseline averaged several cells and scored about $-0.64$ hidden log-loss. A stronger v2 baseline preferred the deepest populated cell, especially subject-benchmark-condition, with Bayesian shrinkage toward coarser cells. This improved local NLL dramatically, but hidden Codabench revealed that the finest cells were less stable under distribution shift than local validation suggested. The final successful baseline kept this smoothed-prior backbone and used revealed labels only through a tightly clipped inferred data-category offset.
...
\texttt{baseline\_clip005} & $-0.62$ & -- & Current baseline with a tightly clipped global adaptive offset. \\
...
\texttt{baseline\_category\_offset} & $-0.60$ & -- & Best submission: v2 smoothed prior plus inferred category-level adaptive calibration. \\
\bottomrule
\end{tabularx}
\caption{Selected Codabench results, ordered from lower to higher hidden Codabench negative log-loss score. Higher is better because scores are negative. The best \texttt{parism} entry was $-0.60$.}
\label{tab:history}
\end{table}

The main ablation pattern was consistent. First, calibration and shrinkage mattered more than model complexity. The factor model had useful discriminative signal on some benchmarks, but it was poorly calibrated on others and was often dominated by smoothed priors. Second, adaptive labels helped only when used at the right level. Removing the label blend scored $-0.63$, a small global offset scored $-0.62$, and inferred category-level offsets improved to $-0.60$. Third, local simulation was useful for screening variants, but it was not a perfect proxy: increasing the default v2 prior weight from 0.45 to 0.60 improved several public-data simulator seeds, yet dropped to $-0.63$ on hidden Codabench.
...
The third failure mode was adaptive-label selection bias. Selecting the longest items produced revealed labels concentrated in hard coding/math categories. A global offset then applied those hard-item residuals to unrelated predictions. Very fine benchmark-condition offsets also overfit because $K=5$ labels are too few to estimate a stable correction without shrinkage. Inferred category-level offsets were the useful compromise: broad enough to transfer labels, but specific enough to capture systematic category shifts. The final lesson is that this challenge rewards robust, boring probability estimation. The winning path for my submission was not a larger encoder or more expressive model, but a conservative prior hierarchy, defensive runtime code, stable acquisition, and clipped category-level calibration.

\section{Reproducibility}

\begin{flushleft}
The code is available at \url{https://github.com/paris007/torch_measure/tree/predictive-eval-challenge}. The final Codabench-ready directories live under \texttt{codabench\_submissions/}; \texttt{make\_submission.py} builds flat ZIP archives with \texttt{model.py} at the archive root. The best submitted artifact for archival purposes is \texttt{baseline\_category\_offset\_submission.zip}; the best leaderboard entry is username \texttt{parism}, score $-0.60$, submission ID 746268.
\end{flushleft}
...
Baseline adaptive ablations & $-0.60$--$-0.63$ & No adaptive offset scored $-0.63$; clipped global offset scored $-0.62$; inferred category-level offset scored the best result, $-0.60$. \\
\bottomrule
\end{longtable}
\normalsize

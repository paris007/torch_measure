#!/usr/bin/env python3
"""Create last-mile calibration Codabench ZIP variants around the winning category-offset submission.

How to use:
1. Save this file as projects/predictive_eval_challenge/create_calibration_variants.py
2. From the repo root, run:
       python projects/predictive_eval_challenge/create_calibration_variants.py
3. Upload the ZIPs printed at the end from:
       projects/predictive_eval_challenge/dist/calibration_variants/

These variants keep the exact winning category-offset structure and only change
one final calibration step. They are designed for log-loss tuning, not for
...
# The first three soften predictions on the logit scale. The last two test a
# tiny global probability shift after the winning category-offset calibration.
VARIANTS = {
    "baseline_category_offset_temp090": dict(logit_scale=0.90, logit_bias=0.0, prob_shift=0.0),
    "baseline_category_offset_temp095": dict(logit_scale=0.95, logit_bias=0.0, prob_shift=0.0),
    "baseline_category_offset_temp085": dict(logit_scale=0.85, logit_bias=0.0, prob_shift=0.0),
    "baseline_category_offset_shift_down002": dict(logit_scale=1.00, logit_bias=0.0, prob_shift=-0.02),
    "baseline_category_offset_shift_up002": dict(logit_scale=1.00, logit_bias=0.0, prob_shift=0.02),
}
...
def main() -> None:
    ...
    dist = project / "dist" / "calibration_variants"
    base_artifact = sub_root / "baseline" / "artifacts" / "smoothed_prior.json"
    dist.mkdir(parents=True, exist_ok=True)

    print("Creating calibration variants around baseline_category_offset...\n")
    ...
    order = [
        "baseline_category_offset_temp095_submission.zip",
        "baseline_category_offset_temp090_submission.zip",
        "baseline_category_offset_temp085_submission.zip",
        "baseline_category_offset_shift_down002_submission.zip",
        "baseline_category_offset_shift_up002_submission.zip",
    ]

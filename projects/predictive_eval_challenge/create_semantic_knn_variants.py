...
# Semantic-kNN adaptive residual variants:
# Keep the winning smoothed-prior + category-offset backbone, then use
# MiniLM embeddings to borrow residuals from semantically similar revealed labels.
...
VARIANTS={
 "baseline_semknn_prob_a10":      dict(MODE="prob", ALPHA=0.10, TOPK=4, TAU=0.08, MINSIM=0.20, CLIP=0.10, SALT="semknn_prob_a10_parism"),
 "baseline_semknn_prob_a20":      dict(MODE="prob", ALPHA=0.20, TOPK=4, TAU=0.08, MINSIM=0.20, CLIP=0.10, SALT="semknn_prob_a20_parism"),
 "baseline_semknn_logit_a10":     dict(MODE="logit",ALPHA=0.10, TOPK=4, TAU=0.08, MINSIM=0.20, CLIP=0.80, SALT="semknn_logit_a10_parism"),
 "baseline_semknn_logit_a20":     dict(MODE="logit",ALPHA=0.20, TOPK=4, TAU=0.08, MINSIM=0.20, CLIP=0.80, SALT="semknn_logit_a20_parism"),
 "baseline_semknn_prob_a15_strict": dict(MODE="prob", ALPHA=0.15, TOPK=3, TAU=0.06, MINSIM=0.35, CLIP=0.08, SALT="semknn_prob_a15_strict_parism"),
 "baseline_semknn_logit_a15_strict":dict(MODE="logit",ALPHA=0.15, TOPK=3, TAU=0.06, MINSIM=0.35, CLIP=0.60, SALT="semknn_logit_a15_strict_parism"),
}

def find_project():
    here=Path.cwd().resolve()
    for p in [here, here/"projects"/"predictive_eval_challenge", Path(__file__).resolve().parent]:
        if (p/"codabench_submissions"/"baseline"/"artifacts"/"smoothed_prior.json").exists(): return p
    raise FileNotFoundError("Run from repo root or put script in projects/predictive_eval_challenge.")
...
def main():
    ...
    artifact=subroot/"baseline"/"artifacts"/"smoothed_prior.json"
    for name,cfg in VARIANTS.items():
        ...
        lab=LABEL.replace("__SALT__", cfg["SALT"])
        (dst/"model.py").write_text(code.lstrip(), encoding="utf-8")
        (dst/"labeling.py").write_text(lab.lstrip(), encoding="utf-8")
        ...
        out=dist/f"{name}_submission.zip"; zipdir(dst,out); print("READY:",out)
    print("\nUpload once each:")
    for i,n in enumerate(VARIANTS,1): print(f"{i}. {n}_submission.zip")
    print("\nIf one gets -0.59/-0.58, rerun it. If all worse than -0.60, keep category_offset/salt10.")

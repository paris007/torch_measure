#!/usr/bin/env python3
"""Create adaptive logit-residual variants around baseline_category_offset.

These variants keep the same smoothed-prior artifact and acquisition function,
but convert the few revealed labels into conservative log-odds corrections.
That matches the usual ability-minus-difficulty view better than adding raw
...
VARIANTS = [
    {
        "name": "baseline_category_logit_offset",
        "w_global": 0.25,
        "w_category": 0.50,
        "w_bc": 0.25,
        "w_subject_category": 0.00,
        "clip": 0.22,
        "shrink_n": 5.0,
    },
    {
        "name": "baseline_category_logit_subject_tiny",
        "w_global": 0.25,
        "w_category": 0.45,
        "w_bc": 0.15,
        "w_subject_category": 0.15,
        "clip": 0.22,
        "shrink_n": 6.0,
    },
    {
        "name": "baseline_category_logit_subject_medium",
        "w_global": 0.20,
        "w_category": 0.40,
        "w_bc": 0.10,
        "w_subject_category": 0.30,
        "clip": 0.25,
        "shrink_n": 7.0,
    },
    {
        "name": "baseline_category_logit_subject_plus_global",
        "w_global": 0.35,
        "w_category": 0.35,
        "w_bc": 0.05,
...
W_BC = {w_bc}
W_SUBJECT_CATEGORY = {w_subject_category}
...
def main() -> None:
    ...
    for variant in VARIANTS:
        ...
        model = MODEL_TEMPLATE.format(**variant)
        (target / "model.py").write_text(textwrap.dedent(model).lstrip())

"""Codabench logit-mean ensemble of the factor / PGE and smoothed-prior models.

The factor model brings discrimination (AUC ~0.70) while the smoothed prior is
extremely well-calibrated to the actual per-(benchmark, condition) base rate.
Averaging the two in logit space combines those complementary strengths.
...
def _baseline_predict(row: Mapping[str, object]) -> float:
    """v2 baseline: prefer the deepest populated cell, fall back upward.

    See `baseline/model.py` for full rationale. The 4-way
    (subject, benchmark, condition) cell is the lowest-variance unbiased
    estimate of P(correct) for new items in that cell.
    """
...
if FACTOR_PATH.exists():
    try:
        ...
        ENCODER = SentenceTransformer(str(FACTOR_ARTIFACT["encoder_id"]))
...
def _base_predict(row: Mapping[str, object]) -> float:
    ...
    baseline_p = _baseline_predict(row)
    if FACTOR_ARTIFACT is None or ENCODER is None or not MLP_LAYERS:
        return baseline_p
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    v6: trust the stored cold-start T (=0.275 by recal) and add an
    EB-shrunk per-category residual offset. Uniform labeling.py makes
    the offsets unbiased; ±0.08 clipping bounds damage.
    """
    try:

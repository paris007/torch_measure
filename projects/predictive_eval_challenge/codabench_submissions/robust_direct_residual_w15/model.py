"""Codabench robust direct-residual ensemble.

This variant keeps the robust prior/router/adaptive-labeling shell that has
tied our best hidden score, then adds a compact learned residual model:

    logit(p) = logit(rich_prior) + residual(item_embedding, prior_features)

The residual is blended conservatively with the prior in logit space. If the
encoder or artifact cannot be loaded, prediction degrades to the proven prior
...
DIRECT_ENCODER = None
DIRECT_LAYERS: list[tuple[np.ndarray, np.ndarray]] = []
DIRECT_FEATURE_MEAN = None
DIRECT_FEATURE_SCALE = None
DIRECT_TEMPERATURE = 1.0
DIRECT_RESIDUAL_SCALE = 0.5
DIRECT_GLOBAL_MEAN = float(BASELINE.get("global_mean", 0.5))

if DIRECT_PATH.exists():
    try:
        DIRECT_ARTIFACT = np.load(DIRECT_PATH, allow_pickle=False)
        DIRECT_FEATURE_MEAN = np.asarray(DIRECT_ARTIFACT["feature_mean"], dtype=np.float32)
        DIRECT_FEATURE_SCALE = np.asarray(DIRECT_ARTIFACT["feature_scale"], dtype=np.float32)
        DIRECT_FEATURE_SCALE = np.where(
            DIRECT_FEATURE_SCALE < 1e-6,
            1.0,
            DIRECT_FEATURE_SCALE,
        ).astype(np.float32)
        DIRECT_TEMPERATURE = float(DIRECT_ARTIFACT["temperature"])
        DIRECT_RESIDUAL_SCALE = float(DIRECT_ARTIFACT["residual_scale"])
        DIRECT_GLOBAL_MEAN = float(DIRECT_ARTIFACT["global_mean"])
        layer_indices = sorted(
            int(k[len("mlp_w") :])
            for k in DIRECT_ARTIFACT.files
            if k.startswith("mlp_w")
        )
        DIRECT_LAYERS = [
            (
                np.asarray(DIRECT_ARTIFACT[f"mlp_w{i}"], dtype=np.float32),
                np.asarray(DIRECT_ARTIFACT[f"mlp_b{i}"], dtype=np.float32),
            )
            for i in layer_indices
        ]
        from sentence_transformers import SentenceTransformer

        DIRECT_ENCODER = SentenceTransformer(
            str(DIRECT_ARTIFACT["encoder_id"]),
            local_files_only=True,
        )
        DIRECT_ENCODER.max_seq_length = MAX_SEQ_LENGTH
    except Exception as exc:  # noqa: BLE001
        print(f"[direct] could not initialize residual model: {exc}", flush=True)
        DIRECT_ARTIFACT = None
        DIRECT_ENCODER = None
        DIRECT_LAYERS = []


def _direct_prior_features(row: Mapping[str, object]) -> tuple[np.ndarray, float]:
    """Feature vector and raw base logit used by the trained residual MLP."""
    g = float(BASELINE.get("global_mean", DIRECT_GLOBAL_MEAN))
    subject_name = _parse_subject_name(row.get("subject_content"))
    benchmark = str(row.get("benchmark", ""))
    condition = str(row.get("condition", "none") or "none")

    s_val = float(BASELINE.get("subject", {}).get(subject_name, g))
    b_val = float(BASELINE.get("benchmark", {}).get(benchmark, g))
    bc_val = float(BASELINE.get("benchmark_condition", {}).get(_key(benchmark, condition), g))
    sb_val = float(BASELINE.get("subject_benchmark", {}).get(_key(subject_name, benchmark), g))
    sbc_raw = BASELINE.get("subject_benchmark_condition", {}).get(
        _key(subject_name, benchmark, condition)
    )
    sbc_val = float(sbc_raw) if sbc_raw is not None else sb_val

    p1 = 0.85 * (0.35 * s_val + 0.15 * b_val + 0.25 * bc_val + 0.25 * sb_val) + 0.15 * g
    p2 = 0.15 * p1 + 0.85 * (0.95 * sbc_val + 0.05 * g)
    base = 0.8 * p1 + 0.2 * p2
    vals = np.asarray([p1, p2, base, s_val, b_val, bc_val, sb_val, sbc_val], dtype=np.float32)
    vals = np.clip(vals, EPS, 1.0 - EPS)
    logits = np.asarray([_logit(float(v)) for v in vals], dtype=np.float32)
    flags = np.asarray(
        [
            1.0 if sbc_raw is not None else 0.0,
            1.0 if condition and condition != "none" else 0.0,
        ],
        dtype=np.float32,
    )
    return np.concatenate([logits, flags]).astype(np.float32), float(logits[2])


def _direct_predict(row: Mapping[str, object]) -> float:
    if (
        DIRECT_ARTIFACT is None
        or DIRECT_ENCODER is None
        or not DIRECT_LAYERS
        or DIRECT_FEATURE_MEAN is None
        or DIRECT_FEATURE_SCALE is None
    ):
        return _clip_probability(DIRECT_GLOBAL_MEAN)
    try:
        embedding = DIRECT_ENCODER.encode(
            [_format_item_text(row)],
            normalize_embeddings=True,
            show_progress_bar=False,
            convert_to_numpy=True,
        ...
        embed_arr = np.asarray(embedding, dtype=np.float32).reshape(-1)
        prior_feats, base_logit = _direct_prior_features(row)
        features = np.concatenate([embed_arr, prior_feats]).astype(np.float32)
        h = ((features - DIRECT_FEATURE_MEAN) / DIRECT_FEATURE_SCALE).reshape(1, -1)
        for idx, (w, bias) in enumerate(DIRECT_LAYERS):
            h = h @ w.T + bias
            if idx < len(DIRECT_LAYERS) - 1:
                h = _gelu(h)
        residual = DIRECT_RESIDUAL_SCALE * float(h.ravel()[0])
        T = max(float(DIRECT_TEMPERATURE), 1e-3)
        logit = (base_logit + residual) / T
        return _clip_probability(1.0 / (1.0 + math.exp(-logit)))
    except Exception as exc:  # noqa: BLE001
        print(f"[direct] _direct_predict fallback: {exc}", flush=True)
        return _clip_probability(DIRECT_GLOBAL_MEAN)
...
def _base_predict(
    ...
    if DIRECT_ARTIFACT is not None and DIRECT_ENCODER is not None and DIRECT_LAYERS:
        direct_p = _direct_predict(row)
        w_direct = min(1.0, max(0.0, DIRECT_WEIGHT))
        combined_logit = (1.0 - w_direct) * _logit(baseline_p) + w_direct * _logit(direct_p)
        baseline_p = _clip_probability(1.0 / (1.0 + math.exp(-combined_logit)))
    if DISABLE_FACTOR:
        return baseline_p
    if FACTOR_ARTIFACT is None or ENCODER is None or not MLP_LAYERS:
        return baseline_p

"""Codabench robust v2 prior with prediction-time labels ignored.
...
This is a focused diagnostic variant. Local adaptive-label simulation showed
the v2 prior was best before label correction, while the five revealed labels
could over-steer predictions. The acquisition function is unchanged, but
`predict()` deliberately uses the static prior path.
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    """Return P(correct) while ignoring prediction-time labels."""
    try:
        return _base_predict(input, labeled=None)
    except Exception as exc:  # noqa: BLE001

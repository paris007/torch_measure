...
MAX_ITEM_CHARS = 800
MAX_SEQ_LENGTH = 128
DEFAULT_FACTOR_WEIGHT = 0.7
DEFAULT_V2_PRIOR_WEIGHT = 0.25
...
def _load_npz_model(npz: np.lib.npyio.NpzFile) -> dict[str, object]:
    ...
    return {
        ...
        "temperature": float(npz["temperature"]) if "temperature" in npz.files else 1.0,
        "subject_theta": {
            str(name): float(value)
            for name, value in zip(npz["subject_names"].tolist(), npz["subject_theta"].tolist())
        },
        "mlp_layers": [
            (
        ...
        "encoder_id": str(npz["encoder_id"]) if "encoder_id" in npz.files else None,
    }


def _predict_seed_from_embedding(
    ...
    b = float(h.ravel()[1])
    a = math.exp(log_a)
    subject_theta = model["subject_theta"]
    theta = subject_theta.get(subject_name, float(model["global_theta"]))
    T = float(temperature) if temperature is not None else float(model["temperature"])
    logit = (a * theta - b) / max(T, 1e-3)
    return 1.0 / (1.0 + math.exp(-logit))

seed_paths = sorted(ARTIFACTS.glob("factor_pge_seed*.npz"))
if seed_paths:
    try:
        encoder_id = None
        means: list[float] = []
        for path in seed_paths:
            ...
            model = _load_npz_model(npz)
            SEED_MODELS.append(model)
            means.append(float(model["global_mean"]))
            if encoder_id is None:
                encoder_id = model["encoder_id"]
        if means:
            GLOBAL_MEAN_FACTOR = float(np.mean(means))
        if encoder_id:
            from sentence_transformers import SentenceTransformer

            ENCODER = SentenceTransformer(encoder_id, local_files_only=True)
            ENCODER.max_seq_length = MAX_SEQ_LENGTH
        FACTOR_ARTIFACT = True
        print(f"[ensemble] loaded {len(SEED_MODELS)} factor seeds", flush=True)
    except Exception as exc:  # noqa: BLE001
        print(f"[ensemble] could not initialize multiseed factor: {exc}", flush=True)
        FACTOR_ARTIFACT = None
        SEED_MODELS = []
        ENCODER = None
elif FACTOR_PATH.exists():
    try:
...
def _factor_predict(row: Mapping[str, object], temperature: float | None = None) -> float:
    if FACTOR_ARTIFACT is None or ENCODER is None:
    ...
    try:
        ...
        if SEED_MODELS:
            probs = [
                _predict_seed_from_embedding(model, embed_arr, subject_name, temperature)
...
def _base_predict(
    ...
    if FACTOR_ARTIFACT is None or ENCODER is None or (not MLP_LAYERS and not SEED_MODELS):
        return baseline_p

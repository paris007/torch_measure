...
def acquisition_uncertainty(row: Mapping[str, object], *, spread_weight: float = 0.15) -> float:
    """Higher => more informative for calibration (near 0.5, far from category prior)."""
    base = baseline_predict_v1(row)
    uncertainty = abs(base - 0.5)
    spread = abs(base - category_prior(row))
    score = uncertainty + spread_weight * spread
